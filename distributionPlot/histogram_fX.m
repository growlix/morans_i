function [N,X,sp] = histogram_fX(varargin)
% HISTOGRAM generates a histogram_fX using the "optimal" number of bins
%
% If called with no output argument, histogram_fX plots into the current axes
%
% SYNOPSIS [N,X,sp] = histogram_fX(data,factor,normalize)
%          [...] = histogram_fX(data,'smooth')
%          [...] = histogram_fX(axesHandle,...)
%
% INPUT    data: vector of input data
%          factor: (opt) factor by which the bin-widths are multiplied
%                   if 'smooth' (or 's'), a smooth histogram_fX will be formed.
%                   (requires the spline toolbox). For an alternative
%                   approach to a smooth histogram_fX, see ksdensity.m
%                   if 'discrete' (or 'd'), the data is assumed to be a discrete
%                   collection of values. Note that if every data point is,
%                   on average, repeated at least 3 times, histogram_fX will
%                   consider it a discrete distribution automatically.
%                   if 'continuous' (or 'c'), histogram_fX is not automatically
%                   checking for discreteness.
%          normalize : if 1 (default), integral of histogram_fX equals number
%                       data points. If 0, height of bins equals counts.
%                       This option is exclusive to non-"smooth" histograms
%          axesHandle: (opt) if given, histogram_fX will be plotted into these
...
%
% REMARKS: The smooth histogram_fX is formed by calculating the cumulative
%           histogram_fX, fitting it with a smoothening spline and then taking
...
%           total number of data points. For a standard histogram_fX, the sum
...
%           smooth histogram_fX might get very steep at the beginning and
...
% c: 2/05 jonas
%
% RG: 20170113, replaced all 'histogram' to 'histogram_fX' to avoid
% conficts with native matlab function of the same name
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% test input
if nargin < 1
    error('not enough input arguments for histogram_fX')
end
...
if ischar(factor)
    switch factor
        ...
        case {'continuous','c'}
            factor = -3;
    otherwise
        error('The only string inputs permitted for histogram_fX.m are ''smooth'',''discrete'', or ''continuous''')
    end
else
...
nData = length(data);
% check whether we do a standard or a smooth histogram_fX
if factor ~= -1
    ...
    if factor == -2 || (factor ~= -3 && nBins*3 < nData) 
        ...
        % adjust the height of the histogram_fX
        if normalize
            Z = trapz(xx,nn);
            nn = nn * nData/Z;
        end
