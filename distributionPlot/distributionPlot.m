...
%           reflecting the shape of the histogram_fX of the data. If false,
...
%           0 : every histogram_fX is normalized individually so that the
...
%               shows the left histogram_fX 5x as wide as the right, while
...
%                   been specified.
%       histOpt : histogram_fX type to plot
%                   0 : use hist command (no smoothing, fixed number of
%                       bins)
%                   1 : smoothened histogram_fX using ksdensity with
%                       Normal kernel. Default.
%                   1.1: smoothened histogram_fX using ksdensity where the
%                       kernel is robustly estimated via histogram_fX.m.
%                       Normal kernel.
%                   2 : histogram_fX command (no smoothing, automatic
...
%                       overly smooth histogram_fX. Default: 1/2
...
%                   inverted.
%       histOri: Orientation of histogram_fX. Either 'center', 'left', or
...
%
%
% SEE ALSO histogram_fX, ksdensity, plotSpread, boxplot, grp2idx
%
    ...
    if ~isempty(currentData(:)) && any(isfinite(currentData(:)))
        ...
        switch floor(opt.histOpt)
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    % use histogram_fX to estimate kernel
                    [dummy,x] = histogram_fX(currentData); %#ok<ASGLU>
                    if length(x) == 1
                    ...
                    % x,y are switched relative to normal histogram_fX
                ...
                % modify histogram_fX such that the sum of bins (not the
            ...
            case 2
                % use histogram_fX - bar heights are counts as in hist
                [xHist,yHist] = histogram_fX(currentData,opt.divFactor,0);
            case 99
...
for iData = goodData'
    ...
    if opt.showMM > 0
        ...
        if opt.showMM == 6
            ...
            % ML 2018/01/10 - fixed problem of line is exactly
            % between two different bins
            yArray = round(yArray,4) ;
